...
from utils.datasets import Cub2011Eval
from utils.preprocess import mean, std
from utils.local_parts import id_to_path, id_to_part_loc, id_to_bbox, part_num, in_bbox
...
def get_corresponding_object_parts(ppnet, args, half_size, use_noise=False):
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.test_batch_size, num_workers=0, pin_memory=False, drop_last=False, shuffle=False)
    ...
    for _, (data, targets, img_ids) in tqdm(enumerate(test_loader)):
        ...
        _, proto_dist = ppnet_without_ddp.push_forward(data)
        proto_acts = - proto_dist
    ...
    for test_image_label in tqdm(range(num_classes)):
        ...
        for _, img_id in enumerate(img_ids):
            test_image_path = os.path.join(args.data_path, base_folder, id_to_path[img_id][0], id_to_path[img_id][1])
            # Read the image
            original_img_ori = cv2.imread(test_image_path)
            original_img = cv2.resize(original_img_ori, (img_size, img_size))

            # Get part annotations for CUB images
            ...
            for part_loc in part_locs:
                ...
                loc_x, loc_y = part_loc[1], part_loc[2]
                ratio_x, ratio_y = loc_x / original_img_ori.shape[1], loc_y / original_img_ori.shape[0]  # Fit the bounding boxes' coordinates to the full images
...
@torch.no_grad()
def get_corresponding_object_parts_my(ppnet, args, half_size, topK, use_noise=False):
    ppnet.eval()
    ppnet_without_ddp = ppnet.module if hasattr(ppnet, 'module') else ppnet
    img_size = ppnet_without_ddp.img_size
    proto_per_class = ppnet_without_ddp.num_prototypes_per_class
    ...
    for _, (data, targets, img_ids) in tqdm(enumerate(test_loader)):
        ...
        all_targets.append(targets.cpu())
        all_img_ids.append(img_ids)
    all_proto_acts = torch.cat(all_proto_acts, dim=0).numpy()   # The activation maps of all test images   (5794, 10, 14, 14)
    all_targets = torch.cat(all_targets, dim=0).numpy() # The categories of all test images
    all_img_ids = torch.cat(all_img_ids, dim=0).numpy() # The image ids of all test images
    ...
    for test_image_label in tqdm(range(num_classes)):
        arr_ids = np.nonzero(all_targets == test_image_label)[0]
        class_proto_acts = all_proto_acts[arr_ids] # Get the activation maps of all the images of current class
        img_ids = all_img_ids[arr_ids]  # Get the image ids of all the images of current class     all 30 images

        #########################################################
        class_proto_acts_max = class_proto_acts.max(axis=(2, 3))
        img_ids_topk = class_proto_acts_max.argsort(axis=0)[::-1][0:topK, :]
        #########################################################
        ...
        for proto_idx in range(proto_per_class):
            ...
            for iii, img_idx in enumerate(img_ids_topk[:, proto_idx]):
                part_labels = class_part_labels[img_idx]    # Get the part labels of current image
                activation_map = class_proto_acts[img_idx, proto_idx]   # Get the activation map of current prototype on current image
                upsampled_activation_map = cv2.resize(activation_map, dsize=(img_size, img_size), interpolation=cv2.INTER_CUBIC)
...
def evaluate_purity(ppnet, args, half_size=16, topK=10):

    all_proto_to_part, all_proto_part_mask = get_corresponding_object_parts_my(ppnet, args, half_size, topK)
    ...
    for proto_idx in range(len(all_proto_to_part)):
        proto_to_part = all_proto_to_part[proto_idx]
        mean_part_float = proto_to_part.mean(0).max()
        all_proto_purity.append(mean_part_float)

    all_proto_purity = np.array(all_proto_purity)
    purity_score = all_proto_purity.mean() * 100
    purity_std = all_proto_purity.std() * 100

    return purity_score, purity_std